 ...
 * {@link #getParameters()}. It allows to represent a specific state of the client that can be bookmarked and reopened
 * at a later point in time. <br>
 * <b>HINT:</b><br>
 * For each of your {@link Dialog} you need to define a {@link #getDialogId() dialog-id}. To decouple your navigation
 * from the actual dialogs and their implementation, it is recommended to define them in a central interface (you can
 * also have multiple such interfaces or classes for large modular systems). See {@link DialogConstants} to get started.
 * For {@link Dialog}s may have mandatory or optional {@link #getParameter(String, Class) parameters} it is further
 * recommended to create sub-classes for {@link DialogPlace} or to define a <em>factory</em> for your
 * {@link DialogPlace}s that takes the parameter values as properly typed arguments. This will make refactoring easier
 * if parameters need to be changed. If you are sub-classing {@link DialogPlace} please be <b>aware</b> that your
 * {@link net.sf.mmm.client.ui.base.dialog.DialogController}s are NOT permitted to cast {@link DialogPlace} to your
 * sub-class as the {@link DialogManager#navigateTo(DialogPlace) navigation} can also take place via URL typing or from
 * a bookmark and the framework will NOT know your subclasses. For generic access to your parameters please also define
 * <code>PARAMETER_*</code> constants in your {@link DialogConstants}.
 *
 * <pre>
 ...
 * </pre>
 *
 * If you do not need to {@link DialogManager#openInNewWindow(DialogPlace) open} {@link DialogPlace places} in a new
 * window, you can also combine your places with the {@link DialogManager} and instead of providing create methods you
 * define methods like:
 *
 * <pre>
...
public class DialogPlace {
   ...
   *
   * @param key is the {@link #getParameter(String, Class) name of the parameter}.
   * @param value is the {@link #getParameter(String, Class) value of the parameter}. May be <code>null</code> if
   ...
   * @param parameters is the map with the current parameters.
   * @param key is the {@link #getParameter(String, Class) name of the parameter}.
   * @param value is the {@link #getParameter(String, Class) value of the parameter}. May be <code>null</code> for
   *        generic usage with optional parameter. Then the given {@link Map} is returned unchanged. Use the empty
   *        {@link String} instead of <code>null</code> to prevent this.
   * @param required - if <code>false</code> the given <code>value</code> may be <code>null</code>, <code>true</code> if
   *        the parameter is required and an exception shall be thrown if <code>value</code> is <code>null</code>.
   * @return the given <code>parameters</code> {@link Map}.
  ...
  /**
   * This method gets the {@link Dialog#getId() ID} of the {@link Dialog} identified by this {@link DialogPlace}.
   *
