...
public class FxTabPanel extends FxComposite<TabPane, UiTab> implements UiTabPanel, FxAtomicWidget<TabPane> {
  ...
  private Tab getTab(UiTab child) {
    ...
    return ((FxTab) child).getNativeWidget();
  }
  ...
  public void addChild(UiTab child, int index) {
    ...
    this.nativeWidget.getTabs().add(fxTab.getNativeWidget());
  ...
  public boolean removeChild(UiTab child) {
    ...
    if (removed) {
      this.nativeWidget.getTabs().remove(getTab(child));
    }
    return removed;
  }
  ...
  public UiTab removeChild(int index) {
    ...
    this.nativeWidget.getTabs().remove(index);
  ...
  public UiTab getActiveChild() {
    ...
    Tab tab = this.nativeWidget.getSelectionModel().getSelectedItem();
    if (tab == null) {
      return null;
    }
  ...
  public boolean setActiveChild(UiTab child) {
    ...
    this.nativeWidget.getSelectionModel().select(getTab(child));
    return true;
  }
  ...
  public void setTabVisible(FxTab fxTab, boolean visible) {
    ...
    ObservableList<Tab> tabs = this.nativeWidget.getTabs();
    if (visible) {
      int index = 0;
      for (UiTab uiTab : this.children) {
        if (uiTab == fxTab) {
          tabs.add(index, fxTab.getNativeWidget());
          return;
        } else {
          if (uiTab.isVisible(UiFlagMode.ANY)) {
            index++;
          }
        }
      }
    } else {
      tabs.remove(fxTab.getNativeWidget());
    }
  }
