...
public class GenericColorTest extends Assertions {
  ...
  /**
   * Checks that the given {@link ColorSegment}s match according to {@code matchType}.
   *
   * @param expected is the expected {@link ColorSegment}.
   * @param actual is the current {@link ColorSegment} to compare with {@code expected}.
   * @param matchType is the {@link MatchType}.
   */
  private void checkSegment(ColorSegment<?> expected, ColorSegment<?> actual, MatchType matchType) {
  ...
  /**
   * Checks that all {@link ColorSegment}s of the given {@link GenericColor} match to the given values. This method expects
   * that the four {@link ColorSegment}s used to create the {@link GenericColor} via the given {@link ColorModel} have to be
   ...
   * @param allEqual - {@code true} if all {@link ColorSegment}s have to be equal, {@code false} if
   *        {@link #PRECISION_CLOSE_MATCH} is sufficient for all {@link ColorSegment}s except the four used to create the
   *        color.
  ...
  /**
   * Checks the conversion of {@link GenericColor} {@link GenericColor#of(String) from} and
  ...
  private void checkStringConversion(GenericColor color, ColorModel colorModel) {
    ...
    GenericColor copy = GenericColor.of(colorString);
  ...
  public void testInvert() {
    ...
    checkInvert(GenericColor.of(Color.GREEN), GenericColor.of(Color.MAGENTA));
    checkInvert(GenericColor.of(Color.BLUE), GenericColor.of(Color.YELLOW));
  }
  ...
  public void testLighten() {
    ...
    GenericColor blueViolet = GenericColor.of(Color.BLUE_VIOLET);
    ...
    double g = 0x2B / 255.0;
    double b = 0xE2 / 255.0;
    assertThat(blueViolet.lighten(half)).isEqualTo(GenericColor.of(new Red(r + (1 - r) * 0.5),
        new Green(g + (1 - g) * 0.5), new Blue(b + (1 - b) * 0.5), Alpha.OPAQUE));
  ...
  public void testDarken() {
    ...
    double g = 0x2B / 255.0;
    double b = 0xE2 / 255.0;
    assertThat(blueViolet.darken(half))
        .isEqualTo(GenericColor.of(new Red(r * 0.5), new Green(g * 0.5), new Blue(b * 0.5), Alpha.OPAQUE));
  ...
  public void testEquals() {
    ...
    assertThat(color).isNotEqualTo(GenericColor.of(red, green, blue, alpha));
    assertThat(color).isNotEqualTo(GenericColor.of(red, green, new Blue(1.0), color.getAlpha()));
    assertThat(color).isNotEqualTo(GenericColor.of(red, new Green(1.0), blue, color.getAlpha()));
    assertThat(color).isNotEqualTo(GenericColor.of(new Red(1.0), green, blue, color.getAlpha()));
  }
  ...
  public void testInvalidStrings() {
    ...
    assertThrows(IllegalArgumentException.class, () -> GenericColor.of("rgba"));
    assertThrows(IllegalArgumentException.class, () -> GenericColor.of("rgb(x,y,z)"));
  }
