...
/**
 * A {@link Color} based on {@link AbstractColorFactor factors}. <br>
 * <b>Note:</b><br>
 ...
 * @since 1.0.0
 */
public final class GenericColor {
   ...
   */
  public static GenericColor of(String color) {

    Objects.requireNonNull(color, "color");
    Color namedColor = Color.ofName(color);
    if (namedColor != null) {
      return of(namedColor);
    }
    ...
    try {
      ...
      // "rgb(1,1,1)" = 10
      if (length >= 7) {
        String model = CaseHelper.toUpperCase(color.substring(0, 3));
          ...
          if (endIndex > 0) {
            ...
            if (endIndex > 0) {
              ...
              if (endIndex > 0) {
                ...
                switch (colorModel) {
                  case RGB:
                    return of(new Red(firstSegment), new Green(secondSegment), new Blue(thirdSegment), alpha);
                  case HSL:
                    return of(new Hue(firstSegment), new Saturation(secondSegment), new Lightness(thirdSegment), alpha);
                  case HSV:
                  case HSB:
                    return of(new Hue(firstSegment), new Saturation(secondSegment), new Brightness(thirdSegment),
                        alpha);
                  default:
   ...
   */
  public static GenericColor of(Color color) {
  ...
  /**
   * Creates a {@link GenericColor} from the given {@link ColorSegment}s of {@link ColorModel#RGB}.
   *
   ...
   */
  public static GenericColor of(Red red, Green green, Blue blue, Alpha alpha) {
  ...
  /**
   * Creates a {@link GenericColor} from the given {@link ColorSegment}s of {@link ColorModel#HSB}.
   *
   ...
   * @return the {@link GenericColor}.
   */
  public static GenericColor of(Hue hue, Saturation saturation, Brightness brightness, Alpha alpha) {
  ...
  /**
   * Creates a {@link GenericColor} from the given {@link ColorSegment}s of {@link ColorModel#HSL}.
   *
   * @param hue is the {@link Hue} part.
   * @param saturation is the {@link Saturation} part.
   ...
   * @return the {@link GenericColor}.
   */
  public static GenericColor of(Hue hue, Saturation saturation, Lightness lightness, Alpha alpha) {
  ...
  /**
   * @param type is the {@link ColorSegmentType} identifying the requested {@link ColorSegment}.
   * @return the {@link ColorSegment} of the given {@code type}.
   */
  public ColorSegment<?> getSegment(ColorSegmentType type) {
  ...
  /**
   * @param model the {@link ColorModel} indicating the {@link ColorSegment}s to {@link ColorSegment#invert() invert}. Typically
   *        {@link ColorModel#RGB} to build the complement of the color.
  ...
  public GenericColor invert(ColorModel model) {
    ...
    switch (model) {
      case RGB:
        return of(this.red.invert(), this.green.invert(), this.blue.invert(), this.alpha);
      case HSL:
        return of(this.hue.invert(), this.saturationHsl.invert(), this.lightness.invert(), this.alpha);
      case HSB:
      case HSV:
        return of(this.hue.invert(), this.saturationHsb.invert(), this.brightness.invert(), this.alpha);
      default:
  ...
  public GenericColor lighten(ColorFactor factor) {
    ...
    return of(this.red.increase(factor), this.green.increase(factor), this.blue.increase(factor), this.alpha);
  }
  ...
  public GenericColor darken(ColorFactor factor) {
    ...
    return of(this.red.decrease(factor), this.green.decrease(factor), this.blue.decrease(factor), this.alpha);
  }
  ...
  public String toString(ColorModel colorModel) {
    ...
    StringBuilder buffer = new StringBuilder(CaseHelper.toLowerCase(colorModel.toString()));
    buffer.append("a(");
    buffer.append(getSegment(colorModel.getFirst()));
    buffer.append(',');
    buffer.append(getSegment(colorModel.getSecond()));
    buffer.append(',');
    buffer.append(getSegment(colorModel.getThird()));
    buffer.append(',');
